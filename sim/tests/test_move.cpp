...
TEST_CASE("CMD_MOVE walks a unit to the target cell") {
    SimWorld* s = sim_create(7, 0);                 // unit 1 (worker) starts at cell (5,5)
    SimCommand mv{}; mv.type = CMD_MOVE; mv.player = 1; mv.unit = 1; mv.tx = W(10); mv.ty = W(2);
    sim_push_command(s, &mv, 1);
    sim_advance(s, 200);
    SimSnapshot snap = sim_get_snapshot(s);
    CHECK(C(snap.entities[1].x) == 10);
    CHECK(C(snap.entities[1].y) == 2);
    CHECK(snap.entities[1].state == SIM_STATE_IDLE);   // arrived -> idle
    sim_destroy(s);
}

TEST_CASE("CMD_MOVE around the wall reaches the far side") {
    SimWorld* s = sim_create(7, 0);
    SimCommand mv{}; mv.type = CMD_MOVE; mv.player = 1; mv.unit = 1; mv.tx = W(20); mv.ty = W(10);
    sim_push_command(s, &mv, 1);
    sim_advance(s, 600);
    SimSnapshot snap = sim_get_snapshot(s);
    CHECK(C(snap.entities[1].x) == 20);
    CHECK(C(snap.entities[1].y) == 10);
    sim_destroy(s);
}
...
TEST_CASE("a future CMD_MOVE does not move the unit early") {
    ...
    SimCommand mv{}; mv.type = CMD_MOVE; mv.player = 1; mv.unit = 1; mv.tx = W(15); mv.ty = W(15);
