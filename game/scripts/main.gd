...
@onready var _hud: HudPanel = $HUD/TopBar
@onready var _camera = $Camera     ## untyped: center_on() is script-provided
@onready var _minimap = get_node_or_null("HUD/MinimapFrame/Margin/Rows/Minimap")
...
func _ready() -> void:
	...
	_camera.configure_bounds(Rect2(Vector2.ZERO, Vector2(_map_w, _map_h) * PX_PER_UNIT))
	if _minimap != null:
...
func _draw() -> void:
	...
	for i in n:
		...
		if _selected.has(_ids[i]):
			var select_radius := 27.0 if type_id == TYPE_HQ else 17.0
			var select_color := Color(0.28, 0.88, 1.0, 0.95) if owner_id == MY_PLAYER else Color(1.0, 0.78, 0.25, 0.95)
			var select_fill := select_color
			select_fill.a = 0.08
			draw_circle(p, select_radius, select_fill)
			draw_arc(p, select_radius, 0.0, TAU, 32, select_color, 2.0)
		...
		if owner_id != 0:   # health bar for owned units (skip neutral resource nodes)
			var bar_width := 38.0 if type_id == TYPE_HQ else 24.0
			var bar_lift := 31.0 if type_id == TYPE_HQ else 20.0
			_draw_health_bar(p, _meta[i * 5 + 3], _meta[i * 5 + 4], bar_width, bar_lift)
	_draw_markers()
	_draw_puffs()
...
func _draw_terrain() -> void:
	...
	draw_rect(bounds.grow(8.0), Color(0, 0, 0, 0.42), true)
	for y in _map_h:
		for x in _map_w:
			var tile := Rect2(Vector2(x, y) * cell, Vector2(cell, cell))
			if _passable[y * _map_w + x] != 0:
				var variation := float((x * 37 + y * 17 + x * y * 3) % 5) * 0.004
				draw_rect(tile, Color(0.043 + variation, 0.061 + variation, 0.096 + variation * 1.5), true)
				if (x * 3 + y * 5) % 11 == 0:
					draw_line(tile.position + Vector2(7, 23), tile.position + Vector2(23, 7), Color(0.22, 0.42, 0.55, 0.07), 1.0)
			else:
				draw_rect(tile, Color(0.105, 0.13, 0.20), true)
				draw_rect(tile.grow(-2.0), Color(0.17, 0.20, 0.30), true)
				draw_line(tile.position + Vector2(2, 2), tile.position + Vector2(cell - 2, 2), Color(0.37, 0.45, 0.61, 0.75), 2.0)
				draw_line(tile.end - Vector2(cell - 2, 2), tile.end - Vector2(2, 2), Color(0, 0, 0, 0.35), 2.0)
	var grid := Color(0.35, 0.65, 0.80, 0.055)
	for gx in _map_w + 1:
	...
	draw_rect(bounds, Color(0.28, 0.58, 0.76, 0.55), false, 2.0)
...
func _draw_entity(p: Vector2, type_id: int, owner_id: int, facing: float) -> void:
	...
	if tex != null:
		if type_id == TYPE_WORKER or type_id == TYPE_SOLDIER:
			draw_set_transform(p, facing, Vector2.ONE)
...
func _draw_health_bar(p: Vector2, hp: int, hp_max: int, width: float, lift: float) -> void:
	if hp_max <= 0:
		return
	var frac := clampf(float(hp) / float(hp_max), 0.0, 1.0)
	var tl := p + Vector2(-width * 0.5, -lift)
	draw_rect(Rect2(tl - Vector2.ONE, Vector2(width + 2.0, 6.0)), Color(0.01, 0.015, 0.025, 0.9), true)
	var bar := Color(0.25, 0.88, 0.46) if frac > 0.3 else Color(0.95, 0.28, 0.24)
	draw_rect(Rect2(tl, Vector2(width * frac, 4.0)), bar, true)
	draw_line(tl, tl + Vector2(width * frac, 0), Color(1, 1, 1, 0.32), 1.0)
